...
function getIcon(object)
  ...
  elseif object.icons then
    ...
    for _, i in pairs(object.icons) do 
      retval = retval .. "{" .. i.icon
      if i.icon_size then
        retval = ":" .. tostring(i.icon_size)
      end 
      retval = retval .. "}" 
    end
    if object.icon_size then
      retval = ":" .. tostring(object.icon_size)
    end 
    retval = retval .. "}"
  end
  return "nil"
...
--            log(prototype.name)
            ...
            if enabled_types[t] then
                ...
                for _, entry in pairs(entries[t]) do
                    ...
                    elseif entry == "energy_usage" or entry == "belt_speed" or entry == "max_energy_usage" or entry == "production" then
...
--                log( "{" .. prototype .. "{" .. i.name .. "}," .. get_icon(i) .. "}")
--                log( "{" .. prototype .. "{" .. prototype.name .. "}," .. getIcon(prototype) .. "}")

                printLocalisedName(folder, t .. ".txt",prototype)
            end
        end
    end
